...
col_names = ["route id", "stop id", "sequence", "package id", "scan_status", "time_window.start", "time_window.end", "service seconds", "depth_cm","height_cm","width_cm", "lat","lng","type","zone_id"]
	...
	for stop_point in list(route.keys()):
		if package_data[route_id].get(stop_point, "NA") != "NA":
			for package_id in list(package_data[route_id][stop_point]):
				...
				row = [route_id, stop_point, sequence_data[route_id]["actual"][stop_point], package_id]
				row += package_info
				route_properties = pd.json_normalize(route_data[route_id]["stops"][stop_point])
				route_properties = list(route_properties.iloc[0])
				row += route_properties
				row = [row]
				to_dataframe = pd.DataFrame(row, columns = col_names)
			...
			route_properties = pd.json_normalize(route_data[route_id]["stops"][stop_point])
			route_properties = list(route_properties.iloc[0])
			row = [route_id, stop_point, sequence_data[route_id]["actual"][stop_point], "", "", "","", "","",""]
			row += route_properties
			row = [row]
			to_dataframe = pd.DataFrame(row, columns = col_names)
			df = df.append(to_dataframe, ignore_index = True)
